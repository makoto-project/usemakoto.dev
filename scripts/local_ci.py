...
def is_git_checkout(path: Path) -> bool:
    """Return whether path is a Git checkout, including a linked worktree."""
    if not path.is_dir():
        return False
    result = subprocess.run(
...
def sync_core(core: Path, commit: str) -> None:
    ...
    if not is_git_checkout(core):
        if core.exists() and any(core.iterdir()):
            raise ValueError(f"core path exists but is not a Git checkout: {core}")
    ...
    if git("rev-parse", "HEAD", cwd=core) != commit:
        ...
        subprocess.run(
            ["git", "checkout", "--detach", "--quiet", commit], cwd=core, check=True
        )

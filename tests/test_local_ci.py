...
def test_sync_core_refuses_a_dirty_checkout(
    ...
    core = tmp_path / "core"
    core.mkdir()
    monkeypatch.setattr(local_ci, "is_git_checkout", lambda path: True)
...
def test_sync_core_refuses_dirty_checkout_already_at_the_pin(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    core = tmp_path / "core"
    core.mkdir()
    commit = "d" * 40
    monkeypatch.setattr(local_ci, "is_git_checkout", lambda path: True)
    calls = {"rev-parse": commit, "status": "?? generated.txt"}
    monkeypatch.setattr(local_ci, "git", lambda *a, **k: calls[a[0]])

    with pytest.raises(ValueError, match="local changes"):
        local_ci.sync_core(core, commit)
